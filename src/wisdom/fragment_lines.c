...
// --- DAEDALUS TECHNICAL GUIDANCE RULES ---

static const char* DAEDALUS_KEYWORDS_STRINGS[] = { "strcpy", "strcat", "sprintf", NULL };
static const char* DAEDALUS_KEYWORDS_MEMORY_ARRAYS[] = { "malloc", "realloc", "array", NULL };
static const char* DAEDALUS_KEYWORDS_LOGGING[] = { "printf", "fprintf", "debug", NULL };
static const char* DAEDALUS_KEYWORDS_MATH[] = { "vector", "matrix", "math", NULL };
static const char* DAEDALUS_KEYWORDS_DATA_STRUCTURES[] = { "linked list", "spatial", "search", NULL };
static const char* DAEDALUS_KEYWORDS_STRING_INVALID_CHECK[] = { "NULL ||", "->str == NULL", "->len == 0", NULL }; // For the new linting issue
static const char* DAEDALUS_KEYWORDS_STRCMP_UNSAFE[] = { "strcmp", "->str", NULL }; // For the new strcmp linting issue

static const DaedalusGuidance_t DAEDALUS_GUIDANCE_RULES[] = {
    {
        .keywords = DAEDALUS_KEYWORDS_STRCMP_UNSAFE,
        .recommendation =
            "🔧 DAEDALUS RECOMMENDS: Weigh your strings on the craftsman's scales!\n" METIS_RESET
            "   Use `d_CompareStrings()` or `d_CompareStringToCString()`.\n"
            "   They handle NULLs gracefully, preventing crashes and ensuring safety."
    },
    {
        .keywords = DAEDALUS_KEYWORDS_STRING_INVALID_CHECK,
        .recommendation =
            "🔧 DAEDALUS RECOMMENDS: Use the master key, `d_IsStringInvalid()`.\n" METIS_RESET
            "   It encapsulates all checks (`NULL`, `->str == NULL`, `->len == 0`) into one robust call."
    },
    {
        .keywords = DAEDALUS_KEYWORDS_STRINGS,
        .recommendation =
            "🔧 DAEDALUS RECOMMENDS: Use dString_t for safe string handling:\n" METIS_RESET
    ...
    {
        .keywords = DAEDALUS_KEYWORDS_MEMORY_ARRAYS,
        .recommendation =
            "🔧 DAEDALUS RECOMMENDS: Use dArray_t for safe memory management:\n" METIS_RESET
    ...
    {
        .keywords = DAEDALUS_KEYWORDS_LOGGING,
        .recommendation =
            "🔧 DAEDALUS RECOMMENDS: Use dLog for structured logging:\n" METIS_RESET
            "   d_LogInfo(\"User action completed\");\n"
            "   d_LogErrorF(\"Failed to process %s\", filename);\n"
            "   // Level-based filtering, file output, thread safety"
    },
    {
        .keywords = DAEDALUS_KEYWORDS_MATH,
        .recommendation =
            "🔧 DAEDALUS RECOMMENDS: Use dVec/dMat for mathematical operations:\n" METIS_RESET
            "   dVec3_t result;\n"
    ...
    {
        .keywords = DAEDALUS_KEYWORDS_DATA_STRUCTURES,
        .recommendation =
            "🔧 DAEDALUS RECOMMENDS: Use specialized data structures:\n" METIS_RESET
    ...
    // Keep this as the last, generic recommendation
    {
        .keywords = NULL, // No specific keywords, acts as a fallback
        .recommendation =
            "🔧 DAEDALUS RECOMMENDS: Check the master craftsman's library for pre-built solutions.\n" METIS_RESET
...
static const size_t DAEDALUS_GUIDANCE_RULE_COUNT = sizeof(DAEDALUS_GUIDANCE_RULES) / sizeof(DAEDALUS_GUIDANCE_RULES[0]);
...
const char* get_daedalus_guidance_for_context(const char* context) {
    if (!context) {
        // Fallback to the generic recommendation if context is NULL
        return DAEDALUS_GUIDANCE_RULES[DAEDALUS_GUIDANCE_RULE_COUNT - 1].recommendation;
    }

    for (size_t i = 0; i < DAEDALUS_GUIDANCE_RULE_COUNT; ++i) {
        // The last rule is the generic fallback, it has NULL keywords
        if (DAEDALUS_GUIDANCE_RULES[i].keywords == NULL) {
            continue; // Skip the generic rule until the very end
        }

        const char** keywords = DAEDALUS_GUIDANCE_RULES[i].keywords;
        for (int j = 0; keywords[j] != NULL; ++j) {
            if (strstr(context, keywords[j]) != NULL) {
                return DAEDALUS_GUIDANCE_RULES[i].recommendation;
            }
        }
    }

    // If no specific match, return the generic recommendation (which is the last one in the array)
    return DAEDALUS_GUIDANCE_RULES[DAEDALUS_GUIDANCE_RULE_COUNT - 1].recommendation;
}